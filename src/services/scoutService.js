...
class ScoutService {
  ...
  async initialize(force = false) {
    if (!this.ndk || force) {
      ...
      try {
        ...
        console.log(
          "✅ Scout Service connected to",
          this.defaultRelays.length,
          "default relays",
        );
      } catch (error) {
  ...
  async scoutUser(targetPubkey, progressCallback = null) {
    try {
      ...
      this.cancelled = false;
      console.log(
        "🔍 Starting scout user for pubkey:",
      ...
      if (progressCallback) {
        progressCallback({
          stage: "Fetching user relay list...",
          processed: 0,
          total: 1,
        });
      }
      ...
      const analysisResults = await this.analyzeFollows(
        followList,
        progressCallback,
      );
      ...
      const totalZombies =
        analysisResults.burned.length +
        analysisResults.fresh.length +
        analysisResults.rotting.length +
        analysisResults.ancient.length;
      const zombieScore =
        totalFollows > 0 ? Math.round((totalZombies / totalFollows) * 100) : 0;
      ...
      console.log(
        `🎯 Scout Results: ${totalFollows} follows, ${totalZombies} zombies (${zombieScore}% zombie score)`,
      );
      console.log(
        `📊 Breakdown: ${analysisResults.active.length} active, ${breakdown.fresh} fresh, ${breakdown.rotting} rotting, ${breakdown.ancient} ancient, ${breakdown.burned} burned`,
      );
  ...
  async analyzeFollows(followList, progressCallback = null) {
    console.log(
      `🔍 Starting authentic activity scan for ${followList.length} profiles using authenticated mode logic...`,
    );
    ...
    console.log(
      "🚀 Using enhanced scanning with SMART RETRY + AGGRESSIVE FALLBACK",
    );

    // 1a. Standard activity scanning using scout's cancellation-aware method
    const activityMap = await this.getProfilesActivityScout(
      followList,
      10,
      progressCallback,
    );
    ...
    const rawZombieResults = zombieService.classifyZombies(
      activityMap,
      null,
      null,
    );
    ...
    console.log(
      "🔍 Using authentic immunityService.filterImmuneZombies method",
    );
    await immunityService.init();
    const zombieResults = immunityService.filterImmuneZombies(rawZombieResults);
    const rawCount =
      rawZombieResults.fresh.length +
      rawZombieResults.rotting.length +
      rawZombieResults.ancient.length;
    const filteredCount =
      zombieResults.fresh.length +
      zombieResults.rotting.length +
      zombieResults.ancient.length;
    console.log(
      `🛡️ Authentic immunity filtering: ${rawCount} raw zombies → ${filteredCount} after filtering`,
    );
  ...
  async getProfilesActivityScout(pubkeys, limit = 10, progressCallback = null) {
    ...
    console.log(
      `🔍 Starting enhanced activity scan for ${pubkeys.length} profiles...`,
    );
    ...
    for (let i = 0; i < pubkeys.length; i += batchSize) {
      ...
      try {
        ...
        const events = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Activity fetch timeout")),
              15000,
            ),
          ),
        ]);
    ...
    console.log(
      `✅ Scout Mode activity scan complete. Found activity for ${Array.from(activityMap.values()).filter((events) => events.length > 0).length}/${pubkeys.length} users`,
    );
    return activityMap;
  }
  ...
  async smartRelayRetry(pubkeys, limit = 10, progressCallback = null) {
    ...
    console.log(
      `🔄 Simplified retry: Using ${reliableRelays.length} reliable relays for ${pubkeys.length} users`,
    );
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Reliable relay connection timeout")),
            8000,
          ),
        ),
      ]);
      ...
      for (let i = 0; i < pubkeys.length; i += batchSize) {
        ...
        try {
          ...
          const events = await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("Reliable relay query timeout")),
                10000,
              ),
            ),
          ]);
          ...
          console.warn(
            `⚠️ Reliable retry batch ${Math.floor(i / batchSize) + 1} failed: ${error.message}`,
          );
        }
      ...
      console.warn(
        `⚠️ Reliable relay retry completely failed: ${error.message}`,
      );
    }

    // Count successful recoveries
    const recoveredCount = Array.from(retryMap.values()).filter(
      (events) => events.length > 0,
    ).length;
    console.log(
      `✅ Reliable retry recovered ${recoveredCount}/${pubkeys.length} users using stable relays`,
    );
  ...
  classifyZombiesAuthentic(activityData, progressCallback = null) {
    ...
    console.log(
      `🔍 Classifying zombies using authenticated thresholds: active=${conservativeActiveThreshold}d, fresh=${freshThreshold}d, rotting=${rottingThreshold}d`,
    );
    ...
    for (const [pubkey, events] of activityData.entries()) {
      ...
      if (events.length === 0) {
        console.log(
          `💀 User ${pubkey.substring(0, 8)}... is ANCIENT zombie: no activity found`,
        );
      ...
      const daysSinceLastActivity =
        (now - latestEvent.created_at) / (24 * 60 * 60);
      ...
      if (daysSinceLastActivity < conservativeActiveThreshold) {
        console.log(
          `✅ User ${pubkey.substring(0, 8)}... is ACTIVE: last activity ${daysSinceLastActivity.toFixed(1)} days ago (< ${conservativeActiveThreshold} days)`,
        );
        zombies.active.push(zombieInfo);
      } else if (daysSinceLastActivity >= rottingThreshold) {
        console.log(
          `💀 User ${pubkey.substring(0, 8)}... is ANCIENT zombie: ${daysSinceLastActivity.toFixed(1)} days since activity`,
        );
        zombies.ancient.push(zombieInfo);
      } else if (daysSinceLastActivity >= freshThreshold) {
        console.log(
          `🧟‍♂️ User ${pubkey.substring(0, 8)}... is ROTTING zombie: ${daysSinceLastActivity.toFixed(1)} days since activity`,
        );
        zombies.rotting.push(zombieInfo);
      } else {
        console.log(
          `🧟‍♀️ User ${pubkey.substring(0, 8)}... is FRESH zombie: ${daysSinceLastActivity.toFixed(1)} days since activity`,
        );
    ...
    const totalZombies =
      ...
      zombies.ancient.length;
    console.log(
      `✅ Classification complete: ${zombies.active.length} active, ${totalZombies} zombies (${zombies.fresh.length} fresh, ${zombies.rotting.length} rotting, ${zombies.ancient.length} ancient, ${zombies.burned.length} burned)`,
    );
  ...
  async analyzeBatch(pubkeys) {
    try {
      ...
      console.log(
        `🔍 Analyzing batch of ${pubkeys.length} users with comprehensive event search...`,
      );
      ...
      const deletionEvents = Array.from(recentEvents).filter(
        (event) => event.kind === 5,
      );
      ...
      console.log(
        `📊 Processing ${recentEvents.size} events across all kinds...`,
      );
      ...
      console.log(
        `👤 Found activity for ${userActivity.size} out of ${pubkeys.length} users`,
      );
      ...
      console.log(
        `🔍 Scout Mode using conservative thresholds: active=${conservativeActiveThreshold}d, fresh=${freshThreshold}d, rotting=${rottingThreshold}d`,
      );
      ...
      for (const pubkey of pubkeys) {
        ...
        if (deletedUsers.has(pubkey)) {
          console.log(
            `🔥 User ${pubkey.substring(0, 8)}... is BURNED zombie: account marked as deleted`,
          );
          ...
          if (daysSinceActivity < conservativeActiveThreshold) {
            console.log(
              `✅ User ${pubkey.substring(0, 8)}... is ACTIVE: last activity ${daysSinceActivity.toFixed(1)} days ago (< ${conservativeActiveThreshold} days)`,
            );
            results.active.push(userInfo);
          } else if (daysSinceActivity >= rottingThreshold) {
            console.log(
              `💀 User ${pubkey.substring(0, 8)}... is ANCIENT zombie: ${daysSinceActivity.toFixed(1)} days since activity`,
            );
            results.ancient.push(userInfo);
          } else if (daysSinceActivity >= freshThreshold) {
            console.log(
              `🧟‍♂️ User ${pubkey.substring(0, 8)}... is ROTTING zombie: ${daysSinceActivity.toFixed(1)} days since activity`,
            );
            results.rotting.push(userInfo);
          } else {
            console.log(
              `🧟‍♀️ User ${pubkey.substring(0, 8)}... is FRESH zombie: ${daysSinceActivity.toFixed(1)} days since activity`,
            );
    ...
    try {
      ...
      if (profileEvents && profileEvents.size > 0) {
        ...
        try {
          ...
          console.log(
            "✅ Found profile for:",
            profileData.name || profileData.display_name || "unnamed user",
          );
          return {
