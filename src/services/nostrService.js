...
class NostrService {
    ...
    if (!this.ndk) {
      ...
      while (Date.now() - startTime < maxWaitTime) {
        ...
        if (connectedRelays.length > 0) {
          console.log(
            `✅ NDK initialized with ${connectedRelays.length} connected relay(s) (more may connect in background)`,
          );
          break;
        }
  ...
  async connectExtension() {
    ...
    this.connectionPromise = Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(new Error("Overall connection timeout after 30 seconds")),
          30000,
        ),
      ),
    ]).finally(() => {
    ...
    try {
      ...
      try {
        ...
        if (this.userRelayList) {
          ...
          console.log(
            `✅ User relay list loaded: ${writeRelays.length} write, ${readRelays.length} read relays`,
          );
        } else {
      ...
      setTimeout(() => {
        ...
        syncManager
          ...
          .then((result) => {
            if (result.success) {
              console.log(
                `✅ Auto-sync complete: synced ${result.synced.length} services`,
              );
            } else {
              console.warn(
                "⚠️ Auto-sync failed:",
      ...
      if (error.message.includes("timeout")) {
        throw new Error(
          "Extension connection timed out. Please check if your Nostr extension is unlocked and responding.",
        );
      } else if (
        ...
        throw new Error(
          "Connection was denied. Please approve the connection request in your Nostr extension.",
        );
      } else {
        throw new Error(
          `Failed to connect to Nostr extension: ${error.message}`,
        );
      }
    }
  }
  ...
  setSigningMethod(method) {
    ...
    console.log(
      `🔄 Switching signing method from ${this.signingMethod} to ${method}`,
    );
   ...
   */
  isBunkerReady() {
    return this.signingMethod === "nip46" && this.nip46Service.isConnected();
  }
    ...
    try {
        ...
        if (cached) {
          ...
          if (Date.now() - cachedProfile.timestamp < 3600000) {
            ...
            this.userProfile = {
              ...
              display_name:
                cachedProfile.display_name || cachedProfile.displayName || null,
            ...
            if (this.userProfile && this.signingMethod === "nip46") {
              console.log(
                "📡 Dispatching cached user-profile-loaded event for NIP-46",
              );
      ...
      if (eventArray.length > 0) {
        ...
        try {
          ...
          console.warn(
            `Failed to parse user profile JSON: ${jsonError.message}`,
          );
        ...
        this.userProfile = {
          ...
          display_name:
            profileData.display_name || profileData.displayName || null,
  ...
  hexToNpub(hex) {
    try {
      ...
      if (
        ...
        console.warn(
          `Invalid pubkey format: "${hex}" (length: ${hex?.length || 0})`,
        );
        return null;
      }
  ...
  async debugSpecificUser(pubkey) {
    console.log(
      `=== DEBUG: Testing activity detection for ${pubkey.substring(0, 8)}... ===`,
    );
    ...
    for (const query of queries) {
      try {
        ...
        for (const event of events) {
          console.log(
            `  - Kind ${event.kind} at ${new Date(event.created_at * 1000).toISOString()}`,
          );
  ...
  async getProfileMetadata(pubkeys, progressCallback = null) {
    ...
    const validPubkeys = pubkeys.filter((pubkey) => {
      if (
        ...
        console.warn(
          `Skipping invalid pubkey: "${pubkey}" (length: ${pubkey?.length || 0})`,
        );
        return false;
      }
      return true;
    ...
    console.log(
      `Filtered pubkeys: ${validPubkeys.length} valid out of ${pubkeys.length} total`,
    );
    ...
    for (let i = 0; i < validPubkeys.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `Fetching profiles for batch ${batchNumber}/${totalBatches}...`,
        );
        ...
        for (const [pubkey, userEvents] of profileEventsByUser) {
          try {
            ...
            try {
              ...
              console.warn(
                `Failed to parse profile JSON for ${pubkey.substring(0, 8)}...: ${jsonError.message}`,
              );
              ...
              for (let i = userEvents.length - 1; i >= 0; i--) {
                try {
                  ...
                  try {
                    ...
                    console.warn(
                      `Failed to parse deletion timeline JSON for ${pubkey.substring(0, 8)}...: ${jsonError.message}`,
                    );
                    continue; // Skip this event if JSON is invalid
                  }
                  const wasDeleted =
                    eventProfile.deleted === true ||
                    eventProfile.deleted === "true";
                  if (wasDeleted) {
                    deletionTimeline = {
                      markedDeletedAt: userEvents[i].created_at,
                      deletionAge: Math.floor(
                        Date.now() / 1000 - userEvents[i].created_at,
                      ),
                  ...
                  console.warn(
                    `Failed to parse profile event for deletion timeline: ${pubkey}`,
                  );
                }
              }
            }
            ...
            const updatedProfile = {
              ...
              display_name:
                latestProfile.display_name || latestProfile.displayName || null,
            ...
            if (deletionTimeline) {
              console.log(
                `🔥 User ${pubkey.substring(0, 8)}... marked deleted ${Math.floor(deletionTimeline.deletionAge / (24 * 60 * 60))} days ago, ${deletionTimeline.profileUpdatesAfterDeletion} profile updates since`,
              );
            }
          } catch (error) {
        ...
        console.error(
          `Failed to fetch profiles for batch starting at ${i}:`,
          error,
        );
      }
    }
  ...
  async getFollowList() {
    ...
    const validFollowList = rawFollowList.filter((pubkey) => {
      if (!pubkey || typeof pubkey !== "string") {
        console.warn(
          `Filtering out non-string follow entry: ${typeof pubkey} "${pubkey}"`,
        );
        return false;
      }
      ...
      if (pubkey.length !== 64) {
        console.warn(
          `Filtering out invalid length pubkey: "${pubkey}" (length: ${pubkey.length})`,
        );
        return false;
      }
    ...
    if (filteredCount > 0) {
      console.warn(
        `⚠️ Filtered out ${filteredCount} invalid entries from follow list`,
      );
    }
  ...
  async getProfilesActivity(pubkeys, limit = 10, progressCallback = null) {
    ...
    for (let i = 0; i < pubkeys.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `📡 Batch ${batchNum}/${totalBatches}: Scanning ${batch.length} users (balanced approach)...`,
        );
        ...
        try {
          ...
          console.warn(
            `⚠️ Batch ${batchNum} failed: ${error.message}, continuing...`,
          );
    ...
    const activeProfiles = Array.from(activityMap.values()).filter(
      (events) => events.length > 0,
    ).length;
    console.log(
      `📊 Balanced activity scan complete. Found activity for ${activeProfiles} out of ${pubkeys.length} profiles.`,
    );
  ...
  async aggressiveActivityRetry(pubkeys, progressCallback = null) {
    console.log(
      `🔍 AGGRESSIVE RETRY: Attempting to find activity for ${pubkeys.length} users with unknown activity`,
    );
    ...
    for (let i = 0; i < pubkeys.length; i++) {
      ...
      console.log(
        `🔍 RETRY STRATEGY 1: Individual search for ${pubkey.substring(0, 8)}...`,
      );
      ...
      try {
        ...
        if (events.size > 0) {
          console.log(
            `🎉 RETRY SUCCESS: Found ${events.size} events for ${pubkey.substring(0, 8)}... using individual search`,
          );
        }
      } catch (error) {
        console.warn(
          `Individual search failed for ${pubkey.substring(0, 8)}...:`,
          error.message,
        );
      }
    ...
    const stillEmpty = pubkeys.filter(
      (pubkey) => (retryResults.get(pubkey) || []).length === 0,
    );

    if (stillEmpty.length > 0) {
      console.log(
        `🔍 RETRY STRATEGY 2: Basic post search for ${stillEmpty.length} remaining users`,
      );
      ...
      for (let i = 0; i < stillEmpty.length; i += basicBatchSize) {
        ...
        try {
          ...
          if (events.size > 0) {
            console.log(
              `🎉 RETRY SUCCESS: Found ${events.size} basic posts in batch ${Math.floor(i / basicBatchSize) + 1}`,
            );
          }
        } catch (error) {
    ...
    const foundActivity = Array.from(retryResults.values()).filter(
      (events) => events.length > 0,
    ).length;
    console.log(
      `🔍 AGGRESSIVE RETRY COMPLETE: Found activity for ${foundActivity}/${pubkeys.length} users`,
    );
  ...
  async createUnfollowEvent(pubkeysToRemove) {
    ...
    const userFollowTags = mostRecentEvent.tags.filter(
      (tag) => tag[0] === "p" && tag[1],
    );
    const nonUserTags = mostRecentEvent.tags.filter((tag) => tag[0] !== "p");

    console.log(
      `Current tags: ${userFollowTags.length} user follows, ${nonUserTags.length} other tags (topics, relays, etc.)`,
    );
    console.log(
      "Non-user tags:",
      nonUserTags.map((tag) => `${tag[0]}:${tag[1] || ""}`).join(", "),
    );

    // Remove the specified pubkeys from user follow tags only
    const newUserFollowTags = userFollowTags.filter(
      (tag) => !pubkeysToRemove.includes(tag[1]),
    );
    console.log(
      `Filtered user follows: ${userFollowTags.length} -> ${newUserFollowTags.length} (removed ${userFollowTags.length - newUserFollowTags.length})`,
    );
    ...
    console.log(
      `Final tag count: ${tags.length} (${newUserFollowTags.length} user follows + ${nonUserTags.length} preserved tags)`,
    );

    // Check if the event would be too large
    if (newUserFollowTags.length > 1000) {
      console.warn(
        "⚠️ Large follow list detected:",
        newUserFollowTags.length,
        "follows",
      );
      console.warn(
        "⚠️ Some Nostr extensions may have trouble with very large follow lists",
      );
      console.warn(
        "⚠️ If signing fails, you may need to unfollow users in smaller batches",
      );
    }
    ...
    if (estimatedSize > 1024 * 1024) {
      // 1MB
      throw new Error(
        `Follow list too large (${Math.round(estimatedSize / 1024)} KB). Please unfollow users in smaller batches.`,
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ Event published to ${publishResults.successful}/${this.getPublishRelays().length} relays`,
      );
      ...
      if (this.signingMethod === "nip07") {
        ...
        if (
          ...
          signingError.message.toLowerCase().includes("user rejected")
        ) {
          throw new Error(
            "You rejected the signing request. Please try again and approve the signing prompt to update your follow list.",
          );
        } else if (
          signingError.message &&
          signingError.message.toLowerCase().includes("timeout")
        ) {
          throw new Error(
            "Signing request timed out. Your extension may not be responding. Please check if your extension is unlocked and try again.",
          );
        } else if (
          signingError.message.includes("not authorized") ||
          signingError.message.includes("permission")
        ) {
          throw new Error(
            "Extension permissions not granted. Please check your Nostr extension settings for this site.",
          );
        } else {
          throw new Error(
            `Failed to sign follow list update: ${signingError.message}. Check your Nostr extension is unlocked and permissions are granted.`,
          );
        }
      } else if (this.signingMethod === "nip46") {
        ...
        if (
          signingError.message &&
          signingError.message.toLowerCase().includes("rejected")
        ) {
          throw new Error(
            "Signing was rejected in your bunker app. Please approve the signing request to update your follow list.",
          );
        } else if (
          signingError.message &&
          signingError.message.toLowerCase().includes("timeout")
          ...
          throw new Error(
            "Signing request timed out. Please check your bunker app is responding.",
          );
        } else {
          throw new Error(
            `Failed to sign follow list update: ${signingError.message}. Check your bunker connection.`,
          );
        }
      } else {
        throw new Error(
          `Failed to sign follow list update: ${signingError.message}`,
        );
      }
    }
  }
          ...
          if (typeof window.nostr !== "undefined") {
            try {
              ...
              if (currentPubkey === this.pubkey) {
                ...
                setTimeout(() => {
                  console.log(
                    "🔄 Triggering automatic relay sync on session restore...",
                  );
                  syncManager
                    .syncAll()
                    .then((result) => {
  ...
  async restoreNip46Session() {
    try {
      ...
      if (restored) {
        ...
        setTimeout(() => {
          console.log(
            "🔄 Triggering automatic relay sync on NIP-46 session restore...",
          );
          syncManager
            .syncAll()
            .then((result) => {
  ...
  buildRelayListTags(relayList) {
    return [
      ...relayList.bothRelays.map((relay) => ["r", relay]),
      ...relayList.readRelays.map((relay) => ["r", relay, "read"]),
      ...relayList.writeRelays.map((relay) => ["r", relay, "write"]),
    ];
  }

  async addRelayToNip65(relayUrl, { read = true, write = true } = {}) {
    if (!relayUrl) {
    ...
    if (!read && !write) {
      throw new Error("Relay must be added as read, write, or both.");
    }
    ...
    readRelays.delete(normalizedRelay);
    writeRelays.delete(normalizedRelay);
    bothRelays.delete(normalizedRelay);

    if (read && write) {
      bothRelays.add(normalizedRelay);
    } else if (read) {
      readRelays.add(normalizedRelay);
    } else {
      writeRelays.add(normalizedRelay);
    }
    ...
    const event = {
      ...
      tags: this.buildRelayListTags(updatedRelayList),
      content: "",
    };
    ...
    try {
      ...
      this.userRelayList = updatedRelayList;

      return {
        success: true,
        relayList: updatedRelayList,
        publishedToRelays: publishResults.successful,
      };
    } catch (signingError) {
      if (this.signingMethod === "nip07") {
        if (
          ...
          throw new Error(
            "You rejected the signing request. Please approve it to publish your relay list.",
          );
        } else if (
          signingError.message &&
          signingError.message.toLowerCase().includes("timeout")
          ...
          throw new Error(
            "Signing request timed out. Please check your extension and try again.",
          );
        } else if (
          signingError.message.includes("not authorized") ||
          signingError.message.includes("permission")
        ...
        throw new Error(
          `Failed to sign relay list update: ${signingError.message}. Check your Nostr extension is unlocked and permissions are granted.`,
        );
      } else if (this.signingMethod === "nip46") {
        if (
          signingError.message &&
          ...
          throw new Error(
            "Signing was rejected in your bunker app. Please approve the signing request to publish your relay list.",
          );
        } else if (
          signingError.message &&
          signingError.message.toLowerCase().includes("timeout")
        ...
        throw new Error(
          `Failed to sign relay list update: ${signingError.message}. Check your bunker connection.`,
        );
      }

      throw new Error(
        `Failed to sign relay list update: ${signingError.message}`,
      );
    }
  }
  ...
  async fetchRelayList(pubkey) {
    ...
    try {
      ...
      console.log(
        `🔍 Fetching NIP-65 relay list for ${pubkey.substring(0, 8)}...`,
      );
      ...
      try {
        ...
        if (events && events.size > 0) {
          ...
          console.log(
            `✅ Found relay list for ${pubkey.substring(0, 8)}...:`,
            relayList,
          );
          return relayList;
        } else {
          console.log(
            `❌ No relay list found for ${pubkey.substring(0, 8)}...`,
          );
        }
      } catch (fastError) {
        console.warn(
          "⚠️ fetchEvents method failed, falling back to subscription:",
          fastError,
        );
      }
      ...
      if (connectedRelays.length === 0) {
        ...
        while (attempts < maxAttempts) {
          ...
          if (nowConnectedRelays.length > 0) {
            console.log(
              `✅ Connected to ${nowConnectedRelays.length} relays after ${attempts * 500}ms`,
            );
            console.log(
              `📡 Connected relay URLs:`,
              nowConnectedRelays.map((r) => r.url),
            );
            break;
          }
          ...
          if (attempts % 6 === 0) {
            // Log every 3 seconds
            console.log(
              `⏳ Still waiting for relay connections... (${attempts * 500}ms)`,
            );
            ...
            relayStatuses.forEach((relay) => {
              console.log(
                `  📡 ${relay.url}: ${relay.statusText} (${relay.status})`,
              );
            });
          }
        }
        ...
        if (finalConnectedRelays.length === 0) {
          console.warn(
            "⚠️ No relays connected after 3 second fallback timeout",
          );
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          subscription.stop();
          console.log(
            `⏰ Relay list fetch timeout for ${pubkey.substring(0, 8)}...`,
          );
        ...
        subscription.on("error", (error) => {
          ...
          subscription.stop();
          console.error(
            `❌ Error fetching relay list for ${pubkey.substring(0, 8)}...:`,
            error,
          );
      ...
      console.error(
        `❌ Failed to fetch relay list for ${pubkey.substring(0, 8)}...:`,
        error,
      );
      return null;
    }
  ...
  async fetchFollowsRelayLists(pubkeys, progressCallback = null) {
    ...
    console.log(
      `✅ Fetched relay lists for ${relayLists.size} out of ${pubkeys.length} follows`,
    );
    return relayLists;
  }
   ...
   */
  async getProfilesActivityOptimized(
    pubkeys,
    limit = 10,
    ...
    console.log(
      "🚀 Using optimized activity scanning with per-user relay preferences",
    );
    ...
    for (const pubkey of pubkeys) {
      ...
      try {
        const events = await this.fetchUserActivity(
          pubkey,
          relaysToScan,
          limit,
        );
        if (events.length > 0) {
          ...
          console.log(
            `📊 Found ${events.length} events for ${pubkey.substring(0, 8)}... using ${relaysToScan.length} relays`,
          );
        }
      } catch (error) {
        console.warn(
          `⚠️ Failed to fetch activity for ${pubkey.substring(0, 8)}...:`,
          error.message,
        );
      }
    ...
    const foundActivity = Array.from(activityMap.values()).filter(
      (events) => events.length > 0,
    ).length;
    console.log(
      `✅ Optimized activity scan complete: found activity for ${foundActivity}/${pubkeys.length} users`,
    );
  ...
  async smartRelayRetry(pubkeys, progressCallback = null) {
    ...
    console.log(
      `🎯 Starting smart relay retry for ${pubkeys.length} users with cached relay preferences`,
    );
    ...
    const usersWithRelayLists = pubkeys.filter((pubkey) =>
      this.followsRelayLists.has(pubkey),
    );
    const usersWithoutRelayLists = pubkeys.filter(
      (pubkey) => !this.followsRelayLists.has(pubkey),
    );

    console.log(
      `📊 Smart retry: ${usersWithRelayLists.length} users have relay lists, ${usersWithoutRelayLists.length} don't`,
    );
    ...
    const recoveredCount = Array.from(retryResults.values()).filter(
      (events) => events.length > 0,
    ).length;
    console.log(
      `✅ Smart relay retry complete: recovered ${recoveredCount}/${usersWithRelayLists.length} users with relay preferences`,
    );
  ...
  async fetchUserActivitySingle(pubkey, relays, limit = 5) {
    ...
    try {
      ...
      console.warn(
        `Single user fetch failed for ${pubkey.substring(0, 8)}...:`,
        error,
      );
      return [];
    }
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        subscription.on("error", (error) => {
          ...
          specificNdk.pool.close();
          console.warn(
            `Error in user-specific relay scan for ${pubkey.substring(0, 8)}...:`,
            error,
          );
          resolve(events);
        });
      ...
      console.warn(
        `Failed to create specific relay connection for ${pubkey.substring(0, 8)}...:`,
        error,
      );
      return [];
    }
  ...
  getActivityScanRelays(pubkey) {
    ...
    if (relayList) {
      ...
      if (combinedRelays.length > 0) {
        console.log(
          `📡 Using ${writeRelays.length} write + ${readRelays.length} read relays (${combinedRelays.length} unique) for ${pubkey.substring(0, 8)}...`,
        );
  ...
  async getEnhancedUserActivity(pubkeys, progressCallback = null) {
    console.log(
      `🔍 ENHANCED SCAN: Starting comprehensive activity scan for ${pubkeys.length} users`,
    );
    ...
    for (let i = 0; i < pubkeys.length; i += batchSize) {
      ...
      console.log(
        `🔍 Processing enhanced batch ${batchNumber}/${totalBatches} (${batch.length} users)`,
      );
      ...
      const batchPromises = batch.map((pubkey) =>
        this.queryUserActivityParallel(pubkey),
      );
        ...
        batchResults.forEach((result, index) => {
          ...
          if (result.status === "fulfilled" && result.value) {
            ...
            if (result.value.length > 0) {
              console.log(
                `✅ Enhanced scan found ${result.value.length} events for ${pubkey.substring(0, 8)}...`,
              );
            }
          } else {
            console.warn(
              `⚠️ Enhanced scan failed for ${pubkey.substring(0, 8)}...`,
            );
          }
        });
      } catch (error) {
  ...
  async queryUserActivityParallel(pubkey) {
    ...
    const hasUserSpecificRelays = userSpecificRelays.some(
      (relay) => !defaultRelays.includes(relay),
    );
    ...
    if (hasUserSpecificRelays) {
      ...
      queries.push(
        Promise.race([
          ...
          console.warn(
            `User-specific relay query failed for ${pubkey.substring(0, 8)}...:`,
            error.message,
          );
    ...
    queries.push(
      Promise.race([
        ...
        console.warn(
          `Default relay query failed for ${pubkey.substring(0, 8)}...:`,
          error.message,
        );
        return new Set(); // Return empty set on failure
      }),
    ...
    try {
      ...
      if (combinedEvents.length > 0) {
        console.log(
          `🎯 Parallel query success: Found ${combinedEvents.length} unique events for ${pubkey.substring(0, 8)}... from ${queries.length} relay sets`,
        );
      }
      ...
      console.error(
        `Parallel query failed for ${pubkey.substring(0, 8)}...:`,
        error,
      );
      return [];
    }
